NUT-13: Pay-to-Blockheight (P2BH)
==========================
...
This NUT describes Pay-to-Blockheight (P2BH) which is one kind of spending condition based on [NUT-10][10]'s well-known `Secret`. Using P2BH, we can specify an expiry time for an ecash token that is valid and redeemable until a future blockheight. The spending condition is enforced by the mint.

## Pay-to-Pubkey

[NUT-10][10] Secret `kind: P2BH`

If for a `Proof`, `Proof.secret` is a `Secret` of kind `P2BH`, the proof must be unlocked by providing a witness `Proof.witness` and one or more valid signatures in the array `Proof.witness.signatures`. 

In the basic case, when spending an unexpired token, the mint requires one valid Schnorr signature in `Proof.witness.signatures` on `Proof.secret` by the public key in `Proof.Secret.data`. 
...
{
  "amount": 8, 
  "secret": '["P2PH",{"nonce": "5d11913ee0f92fefdc82a6764fd2457a","data": "819000",}]',
...
To spend a token locked with `P2BH`, the spender needs to include signatures in the spent proofs. We use `libsecp256k1`'s serialized 64 byte Schnorr signatures on the SHA256 hash of the message to sign. The message to sign is the field `Proof.secret` in the inputs. If indicated by `Secret.tags.sigflag' in the inputs, outputs might also require signatures on the message `BlindedMessage.B_`. 
